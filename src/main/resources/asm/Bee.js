    ...
    return {
    	'Bee': {
    		...
    		'transformer': function(classNode) {
				...
				var fn3 = asmapi.mapMethod('func_226422_eP_') // isHiveValid
...
function patch_func_70037_a(obj) {
	...
	if (node) {
		...
		var n2 = "net/minecraft/entity/passive/BeeEntity"
...
// [MC-255743] add call to isTooFar
function patch_func_226422_eP_(obj) {
	var m1 = asmapi.mapMethod('func_226409_eA_') // hasHive
	var m2 = asmapi.mapMethod('func_226437_j_') // isTooFar
	var f1 = asmapi.mapField('field_226369_bI_') // hivePos
	var nn = "net/minecraft/entity/passive/BeeEntity"
	var node = asmapi.findFirstMethodCall(obj, asmapi.MethodType.VIRTUAL, nn, m1, "()Z")
	if (node == null) {
		asmapi.log("ERROR", "Failed to modify BeeEntity: call not found")
		return
	}
	node = node.getNext()
	...
	if (node.getOpcode() == opc.IFNE && node2.getType() == AbstractInsnNode.LABEL) {
		...
		var op4 = new FieldInsnNode(opc.GETFIELD, nn, f1, "Lnet/minecraft/util/math/BlockPos;")
		var op5 = asmapi.buildMethodCall(nn, m2, "(Lnet/minecraft/util/math/BlockPos;)Z", asmapi.MethodType.VIRTUAL)
		...
		asmapi.log("ERROR", "Failed to modify BeeEntity: IFNE/Label not found")
}
